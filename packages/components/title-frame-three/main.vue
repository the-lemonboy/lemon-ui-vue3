<template>
  <div class="l-title-border-three" ref="leTitleBox" :style="` width:${getWidth}px; height:${getHeight}px;`">
    <svg class="l-border-svg-container" :width="getWidth" :height="getHeight">
      ...
      <path filter="url(#filterId2)" fill="none"
        ...
        :stroke="borderColor[0]"></path>

      <path xlink-href="#pathD" mask="url(#mask)" filter="url(#filterId1)" fill="none"
        :d="`M 0 ${getHeight - 30}  L ${getWidth / 2 - getTitleWidth / 2} ${getHeight - 30} ${getWidth / 2 - getTitleWidth / 2 + 30},${getHeight - 5}  ${getWidth / 2 + getTitleWidth / 2 - 30},${getHeight - 5} ${getWidth / 2 + getTitleWidth / 2},${getHeight - 30} ${getWidth},${getHeight - 30}`"
        stroke="#02D6B3"></path>
      <use x="5" y="10" xlink:href="rect1"></use>
      <!-- 左边四边形 -->
      <g>
        <polygon :fill="borderColor[1]"
          :points="`10,${getHeight - 25} 20,${getHeight - 25} 30,${getHeight - 5} 20,${getHeight - 5}`"></polygon>
        <polygon opacity="0.8" :fill="borderColor[1]"
          :points="`25,${getHeight - 25} 35,${getHeight - 25} 45,${getHeight - 5} 35,${getHeight - 5}`"></polygon>
        <polygon opacity="0.6" :fill="borderColor[1]"
          :points="`40,${getHeight - 25} 50,${getHeight - 25} 60,${getHeight - 5} 50,${getHeight - 5}`"></polygon>
      </g>
      <g>
        <!-- 右边四边形 -->
        <polygon :fill="borderColor[1]"
